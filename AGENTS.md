...
- **项目一句话**：中国历史朝代科普静态站（夏→清 21 朝），一页看帝王世系 / 人才 / 制度 / 大事记，中英双语、零依赖、零构建。
...
data/
  overview.json         # ★源：21 朝概览 + start/end + counts（驱动 JS-LD / KPI / 存续图）
  dynasties/*.json      # ★源：21 朝详情（帝王 / 人才 / 制度 / 三维）
...
- 2026-09-21：修正朝代数为 **21 朝**（`data/overview.json` / `dynasties/*.json` 实际 21 条）。清扫 README / AGENTS / PRD 中硬编码的「18 朝 / 18 dynasties」现状描述为 21；`check-i18n.mjs` 完成提示改为按 `dynasties` 目录动态计数。保留：模块数（18 个模块 = 17 内容 + 1 首页）、版本号（v1.8 / F18）、PRD 版本历史表中 v2.1 里程碑的「18 朝」（历史记录，不改写）。
