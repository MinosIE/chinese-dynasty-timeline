...
- **F16 同时期政权**：补充辽、西夏、金等政权，共 21 个朝代。
...
- **F20** 单朝代分享卡片 / 海报导出。
- **F21** 繁体中文（zh-Hant）切换。
- **F22** ✅ 朝代疆域缩略图 / 都城分布（SVG 示意地图 + 各朝疆域四至，新增 data/geo.json 源数据）。
- **F23** 世界史横向对照（同期世界文明）。
- **F24** 人物卡片（点击帝王 / 人才看简介）。
...
同一时间只显示一个模块，避免页面超长；跨模块跳转（搜索 / 事件 / 发明）自动切到对应模块。
